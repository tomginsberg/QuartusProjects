/*  CV-8052 bootloader using SPI configuration flash
	...
	Copyright (C) 2009-2015  Jesus Calvino-Fraga, jesusc (at) ece.ubc.ca
...
#define FLASHSECTOR 29 // This one seems available
...
void Manual_Load (void)
{
	...
	while(1)
	{
		...
		else if ( (KEY_1==0) && (KEY_0==0) ) // Save RAM to flash
		{
			while( (KEY_1==0) && (KEY_0==0) ); // Wait for key release
			EraseSector();
			LEDRA_1=1;
...
void main (void)
{
	...
	if( (KEY_0==1) && (KEY_2==1) && (KEY_3==1) ) Load_Ram_Fast_and_Run();
	...
	while(KEY_0==0); // Wait for key release
